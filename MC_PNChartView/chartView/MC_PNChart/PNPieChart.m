...
@property (nonatomic) NSMutableArray<PNPieChartDataItem *> *numberItems;
...
@property (nonatomic) LineLayer *lineLayer;
...
-(id)initWithFrame:(CGRect)frame items:(NSArray *)items{
    ...
    if(self){
        ...
        for (PNPieChartDataItem *item in items) {
            if (item.value > 0) {
                [temp addObject:item];
            }
        }
        [temp sortUsingComparator:^NSComparisonResult(PNPieChartDataItem *_Nonnull obj1, PNPieChartDataItem  *_Nonnull obj2) {
            return obj1.value > obj2.value;
        }];
        
        _numberItems = [self sortArray:temp];
...
- (NSMutableArray *) sortArray:(NSMutableArray *)array {
    ...
    while (count) {
        ...
        [newArray addObject:array.lastObject];
...
- (void)loadDefault{
    ...
    [_numberItems enumerateObjectsUsingBlock:^(PNPieChartDataItem *item, NSUInteger idx, BOOL *stop) {
        if (total == 0){
            [endPercentages addObject:@(1.0 / _numberItems.count * (idx + 1))];
        }else{
...
- (void)recompute {
    ...
    //同理
    CGFloat minimal = (CGRectGetWidth(self.bounds) < CGRectGetHeight(self.bounds)) ? CGRectGetWidth(self.bounds) : CGRectGetHeight(self.bounds)-10;
...
- (void)strokeChart{
    ...
    for (int i = 0; i < _numberItems.count; i++) {
    ...
    if (self.showPullLine) {
        LineLayer *layer = [self pullLineItem];
        layer.backgroundColor = [UIColor clearColor].CGColor;
        layer.frame = _contentView.layer.bounds;
...
//
    ...
    CGPoint center = CGPointMake(self.frame.size.width/2.0 + distance * sin(rad),
                                 self.frame.size.height/2.0  - distance * cos(rad));
...
- (LineLayer *) pullLineItem {
    LineLayer *layer = [LineLayer layer];

    layer.dataItems = self.numberItems;
    layer.cirX = self.bounds.size.width/2.0;
    layer.cirY = self.bounds.size.height/2.0;
//    layer.startAngle = [self startPercentageForItemAtIndex:index];
//    layer.endAngle = [self endPercentageForItemAtIndex:index];
    layer.angels = self.endPercentages;
    layer.radiu = _outerCircleRadius;
    layer.showOnlyValues = self.showOnlyValues;
    layer.showAbsoluteValues = self.showAbsoluteValues;
    layer.hideValues = self.hideValues;
    layer.descriptionTextFont = self.descriptionTextFont;
    return layer;
}
...
- (void)addAnimationIfNeeded{
    if (self.displayAnimated) {
        ...
        if (self.showPullLine) {
            CABasicAnimation *lineAnimation = [CABasicAnimation animationWithKeyPath:@"opacity"];
            lineAnimation.duration = _duration;
...
- (UIView*) getLegendWithMaxWidth:(CGFloat)mWidth{
    ...
    for (PNPieChartDataItem *pdata in self.items) {
        /* Expected label size*/
        NSString *legendTitle = self.showLegendValues ? [NSString stringWithFormat:@"%@ %zi",pdata.textDescription,(long)pdata.value] : pdata.textDescription;
        CGSize labelsize = [PNLineChart sizeOfString:legendTitle
